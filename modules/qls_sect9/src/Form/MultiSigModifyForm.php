...
class MultiSigModifyForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'qls_sect9/multi_sig_modify';

    $form['description'] = [
      '#type' => 'item',
      '#markup' => '9.5 '.$this->t('マルチシグ構成変更'),
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    // \Drupal::logger('qls_sect9')->info('multisig_address: @multisig_address', ['@multisig_address' => $multisig_address]);
    ...
    // \Drupal::logger('qls_sect9')->info('additions: @additions', ['@additions' => print_r($additions, true)]);
    ...
    if (is_array($additions) && count($additions) > 0) {
      foreach ($additions as $addition) {
        \Drupal::logger('qls_sect9')->info('addition: @addition', ['@addition' => $addition]);
        $additions_addresses[] = new UnresolvedAddress($addition);
      }
    }
    // \Drupal::logger('qls_sect9')->info('additions_addresses: @additions_addresses', ['@additions_addresses' => print_r($additions_addresses, true)]); 
    $deletions_addresses = [];
    ...
    // \Drupal::logger('qls_sect9')->info('deletions_addresses: @deletions_addresses', ['@deletions_addresses' => print_r($deletions_addresses, true)]);
