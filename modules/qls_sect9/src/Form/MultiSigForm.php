...
class MultiSigForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'qls_sect9/multi_sig';
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::logger('qls_sect9')->info('cosigners: @cosigners', ['@cosigners' => print_r($cosigners, true)]);
    $cosigner_addresses = [];
    if (is_array($cosigners)) {
        foreach ($cosigners as $cosigner) {
          ...
          // \Drupal::logger('qls_sect9')->info('node_url: @node_url', ['@node_url' => $node_url]);
          $account_info = $accountApi->getAccountInfo($cosigner);
          // \Drupal::logger('qls_sect9')->info('account_info: @account_info', ['@account_info' => $account_info]);
          if ($account_info) {
            // AccountDTOオブジェクトを取得
            $accountDTO = $account_info->getAccount();
            \Drupal::logger('qls_sect9')->info('accountDTO: <pre>@accountDTO</pre>', ['@accountDTO' => print_r($accountDTO,true)]);
            // 公開鍵を取得
            ...
            \Drupal::logger('qls_sect9')->info('account_address: @account_address', ['@account_address' => print_r($account_address,true)]); 
            // $account_address = $accountDTO->getAddress();
            // \Drupal::logger('qls_sect9')->info('account_pubKeyStr: @account_pubKeyStr', ['@account_pubKeyStr' => $account_pubKeyStr]);
            ...
            // \Drupal::logger('qls_sect9')->info('account_address: @account_address', ['@account_address' => $account_address]);
          }
          else {
    ...
    // \Drupal::logger('qls_sect9')->info('cosigner_addresses: @cosigner_addresses', ['@cosigner_addresses' => $cosigner_addresses]);
    // \Drupal::logger('qls_sect9')->info('cosigner_addresses: @cosigner_addresses', ['@cosigner_addresses' => print_r($cosigner_addresses, true)]);
    // 配列を文字列に変換
    ...
    $config = \Drupal::config('qls_sect9.settings');
    // 特定の設定値を取得
    ...
    // \Drupal::logger('qls_sect9')->info('payload: @payload', ['@payload' => $payload]);
