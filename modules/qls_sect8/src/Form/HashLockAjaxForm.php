...
class HashLockAjaxForm extends FormBase {
  ...
  // \Drupal::logger('qls_sect8')->debug('65 Network Type: @network_type', ['@network_type' => $network_type]);
  // \Drupal::logger('qls_sect8')->notice('66 network type:<pre>@object</pre>', ['@object' => print_r($network_type, TRUE)]);
  ...
  public function prompt(array $form, FormStateInterface $form_state) {
    // \Drupal::logger('qls_sect8')->debug('form:<pre>@form</pre>', ['@form' => print_r($form, TRUE)]);
    // ラッパー要素が存在するか確認
    // if (!isset($form['#prefix']) || !isset($form['#suffix'])) {
    //   \Drupal::logger('qls_sect8')->debug('No wrapper found. Adding wrapper.');
    //   $form['#prefix'] = '<div id="aggregate-bounded-transaction-wrapper">';
    //   $form['#suffix'] = '</div>';
    // }else{
    //   \Drupal::logger('qls_sect8')->debug('Wrapper found.');
    // }
    // デバッグ用ログ
    // \Drupal::logger('qls_sect8')->debug('AJAX response triggered. Step: @step', [
    //   '@step' => $form_state->getValue('step'),
    // ]);
    // \Drupal::logger('qls_sect8')->debug('Response data: @response', [
    ...
    // // デバッグ用
    // \Drupal::logger('qls_sect8')->debug('Rendered form: @form', ['@form' => $rendered_form]);
    // return $response;
    $response = new AjaxResponse();
    ...
    if (!$render_context->isEmpty()) {
      // デバッグログを出力
      \Drupal::logger('qls_sect8')->error('Rendering context warnings: @warnings', [
  ...
  public function nextSubmit(array $form, FormStateInterface $form_state) {
    ...
    try {
      ...
      \Drupal::logger('qls_sect8')->error('Exception when calling TransactionRoutesApi->announceTransaction: @message', ['@message' => $e->getMessage()]);
    }
    ...
    \Drupal::logger('qls_sect8')->debug('aggregateTx: @tx', ['@tx' => $facade->hashTransaction($aggregateTx)]);
    ...
    // \Drupal::logger('qls_sect8')->debug('<pre>@form</pre>', ['@form' => print_r($form, TRUE)]);
    // \Drupal::logger('qls_sect8')->debug('Keys: @keys', ['@keys' => array_keys($form_state->getValues())]);
    // \Drupal::logger('qls_sect8')->debug('<pre>@values</pre>', ['@values' => print_r(array_keys($form_state->getValues()), TRUE)]);
...
// \Drupal::logger('qls_sect8')->debug('<pre>@values</pre>', ['@values' => print_r($values, TRUE)]);
    // $network_type = $form_state->getValue('network_type');
    $network_type = $form_state->get('step1_network_type');
    // \Drupal::logger('qls_sect8')->notice('483 network type:<pre>@object</pre>', ['@object' => print_r($network_type, TRUE)]);
    $facade = new SymbolFacade($network_type);
    // ノードURLを設定
    if ($network_type === 'testnet') {
      $networkType = new NetworkType(NetworkType::TESTNET);
    ...
    // \Drupal::logger('qls_sect8')->notice('<pre>@object</pre>', ['@object' => print_r($payload, TRUE)]); 

    // \Drupal::logger('qls_sect8')->notice('<pre>@object</pre>', ['@object' => print_r($networkType, TRUE)]); 
    $config = new Configuration();
    $config->setHost($node_url);
    $client = \Drupal::httpClient();
    $apiInstance = new TransactionRoutesApi($client, $config);
    ...
    try {
      ...
      \Drupal::logger('qls_sect8')->error('トランザクションの発行中にエラーが発生しました: @message', ['@message' => $e->getMessage()]);
    }
