<?php
namespace Drupal\qls_ch5\Plugin\views\query;
...
class MosaicDataSource extends QueryPluginBase {
  ...
  public function execute(ViewExecutable $view) {
    ...
    if (!is_array($flattenedData)) {
      \Drupal::logger('qls_ch5')->error('Mosaic flattened data is not an array.');
      return;
    }
    if (empty($flattenedData)) {
      \Drupal::logger('qls_ch5')->warning('No mosaic data available in state storage.');
      return;
    }
