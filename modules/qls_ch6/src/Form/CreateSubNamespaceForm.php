...
class CreateSubNamespaceForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'qls_ch6/create_subnamespace';
    ...
    foreach ($data as $namespaceInfoDTO) {
      ...
      switch ($depth) {
        ...
        case 2:
          ...
          \Drupal::logger('qls_ch6')->debug('case2 namespace: @namespace1', ['@namespace1' => print_r($namespace1, TRUE)]);
          ...
          $options[$fullNamespaceName] = $fullNamespaceName;
            // 直接マッピング
            // $namespaceMap[$namespaceId] = $namespaceName;
            // \Drupal::logger('qls_ch6')->debug('case2 namespaceMap: @namespaceMap', ['@namespaceMap' => print_r($namespaceMap, TRUE)]);

            // if (isset($namespaceMap[$parentId])) {
            //     // 親ネームスペースの名前を取得して連結
            //     \Drupal::logger('qls_ch6')->debug('case2 parentId: @parentId', ['@parentId' => $parentId]);
