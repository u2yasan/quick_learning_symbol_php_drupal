<?php
namespace Drupal\qls_sect5\Plugin\views\query;
...
class MosaicDataSource extends QueryPluginBase {
  ...
  public function execute(ViewExecutable $view) {
    ...
    if (!is_array($flattenedData)) {
      \Drupal::logger('qls_sect5')->error('Mosaic flattened data is not an array.');
      return;
    }
    if (empty($flattenedData)) {
      \Drupal::logger('qls_sect5')->warning('No mosaic data available in state storage.');
      return;
    }
