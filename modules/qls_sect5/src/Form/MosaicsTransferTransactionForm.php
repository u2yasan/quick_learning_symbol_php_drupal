...
class MosaicsTransferTransactionForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // \Drupal::logger('qls_sect5')->notice('L.182:<pre>@object</pre>', ['@object' => print_r($mosaic_count, TRUE)]); 
    // モザイクIDと対応する金額入力フィールドを動的に生成
    for ($i = 0; $i < $mosaic_count; $i++) {
      // \Drupal::logger('qls_sect5')->notice('L.185:<pre>@object</pre>', ['@object' => print_r($i, TRUE)]); 
      // \Drupal::logger('qls_sect5')->notice('L.187:<pre>@object</pre>', ['@object' => print_r($mosaic_count, TRUE)]); 
      ...
      // \Drupal::logger('qls_sect5')->notice('L.200:<pre>@object</pre>', ['@object' => print_r($form['mosaics_container']['mosaics'][$i]['amount'], TRUE)]); 
      // 特定のプロパティのみをログに出力
      \Drupal::logger('qls_sect5')->notice('L.196: Amount field value: @value', [
    ...
    if (isset($form['mosaics_container']['mosaics'][$mosaic_count - 1]['amount'])) {
      ...
      \Drupal::logger('qls_sect5')->notice('L.235 Amount field value: @value', ['@value' => $amount_title]);
    }
  ...
  public function addMosaicIdAjax(array &$form, FormStateInterface $form_state) {
    ...
    // \Drupal::logger('qls_sect5')->notice('L.329:<pre>@object</pre>', ['@object' => print_r($mosaic_count_debug, TRUE)]); 
    
    $form_state->setRebuild(TRUE);
    // \Drupal::logger('qls_sect5')->notice('<pre>@data</pre>', ['@data' => print_r($form['mosaics_container']['mosaics'], TRUE)]);
    // \Drupal::logger('qls_sect5')->notice('L.332: Amount field value: @value', [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    // \Drupal::logger('qls_sect5')->notice('424 mosaics:<pre>@data</pre>', ['@data' => print_r($mosaics, TRUE)]);
    foreach ($mosaics as $index => $mosaic) {
    ...
    //   //$messageData = $message;
    //   // \Drupal::logger('qls_sect5')->notice('messageData:<pre>@object</pre>', ['@object' => print_r($messageData, TRUE)]);  
    ...
    //     // \Drupal::logger('qls_sect5')->notice('account_info:<pre>@object</pre>', ['@object' => print_r($account_info, TRUE)]);
    //     sleep(1);
