...
class ListNamespacesForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($data as $index => $namespaceInfoDTO) {
      ...
      // \Drupal::logger('qls_sect6')->info('namespaceDTO: <pre>@namespaceDTO</pre>', ['@namespaceDTO' => print_r($namespaceDTO, true)]);
      // if($namespace['namespace']['depth']!=3){//2階層までのネームスペースを取得
      // \Drupal::logger('qls_sect6')->info('depth: @depth', ['@depth' => $namespace['namespace']['depth']]); 
      ...
      // \Drupal::logger('qls_sect6')->info('root_namespaceid: @root_namespaceid', ['@root_namespaceid' => $root_namespaceid]);
      // $responseBody = $this->getNameSpaceNamePostRequest($root_namespaceid, $form_state);
      // \Drupal::logger('qls_sect6')->info('responseBody: @response', ['@response' => $responseBody]);
      ...
      // \Drupal::logger('qls_sect6')->info('namespaces: <pre>@namespaces</pre>', ['@namespaces' => print_r($namespaces, true)]);
      $namespace_name = $namespaces[0]->getName();
      // \Drupal::logger('qls_sect6')->info('namespace_name: @namespace_name', ['@namespace_name' => $namespace_name]);
      ...
      // \Drupal::logger('qls_sect6')->info('alias_type: @alias_type', ['@alias_type' => $alias_type]);
      ...
      {
        case 1:
          // \Drupal::logger('qls_sect6')->info('namespaces: @namespaces', ['@namespaces' => print_r($namespaces, true)]);
    ...
    \Drupal::logger('qls_sect6')->info('namespace_table_data: <pre>@namespace_table_data</pre>', ['@namespace_table_data' => print_r($namespace_table_data, true)]); 
  ...
  //     // 結果をログに記録
  //     // \Drupal::logger('qls_sect6')->info('Response: @response', ['@response' => $body]);

  //     return $body; // 必要に応じて処理
  //   }
  //   catch (RequestException $e) {
  //     // エラーハンドリング
  //     \Drupal::logger('qls_sect6')->error('Error during POST request: @error', ['@error' => $e->getMessage()]);
  //     throw $e;
  //   }
  // }
