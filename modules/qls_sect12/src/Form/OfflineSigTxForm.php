...
class OfflineSigTxForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    // \Drupal::logger('qls_sect12')->info('accountKey_pubKey: @accountKey', ['@accountKey' => $accountKey->publicKey]);
    ...
    // \Drupal::logger('qls_sect12')->info('innerTx: @innerTx', ['@innerTx' => print_r($innerTx, TRUE)]);

    // マークルハッシュの算出
    $embeddedTransactions = $innerTx;
    $merkleHash = $facade->hashEmbeddedTransactions($embeddedTransactions);
    // \Drupal::logger('qls_sect12')->info('Merkle Hash: @merkleHash', ['@merkleHash' => $merkleHash]);
    // アグリゲートTx作成
    ...
    // \Drupal::logger('qls_sect12')->info('Aggregate Tx: @aggregateTx', ['@aggregateTx' => print_r($aggregateTx, TRUE)]);
    ...
    // \Drupal::logger('qls_sect12')->info('Signed Payload: @signedPayload', ['@signedPayload' => print_r($signedPayload['payload'], TRUE)]);
